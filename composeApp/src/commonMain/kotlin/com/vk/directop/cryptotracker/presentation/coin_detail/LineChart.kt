...
import com.vk.directop.cryptotracker.presentation.coin_list.formatLocalDateTime
...
fun LineChart(
    ...
    println("Visible Points: $visibleDataPoints")
    println("selectedDataPoint: $selectedDataPoint")
    println("selectedDataPointIndex: $selectedDataPointIndex")
    println("minYValue: $minYValue, maxYValue: $maxYValue")
    Canvas(modifier = modifier
        ...
        val xLabelTextLayoutResults = visibleDataPoints.map { point ->
            measurer.measure(
                text = point.xLabel, style = textStyle.copy(textAlign = TextAlign.Center)
            )
        }
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (showHelperLines) {
                drawLine(
                    color = if (selectedDataPointIndex == index) {
                        style.selectedColor
                    } else style.unselectedColor, start = Offset(
                        ...
                        style.helperLinesThicknessPx * 2.8f
            ...
            for (i in 1 until drawPoints.size) {
                ...
                val xCoordinate = (p1.x + p0.x) / 2f
                val y1 = p0.y
                val y2 = p1.y

                connectionPoints1.add(DataPoint(xCoordinate, y1, ""))
                connectionPoints2.add(DataPoint(xCoordinate, y2, ""))
            }
            ...
            drawPath(
                path = linePath, color = style.chartLineColor, style = Stroke(width = 3f)
            )
            drawPoints.forEachIndexed { ind, point ->
                if (isShowingDataPoints) {
